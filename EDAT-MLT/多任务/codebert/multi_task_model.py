...
class MultiTaskVulnerabilityModel(nn.Module):
    ...
    def __init__(self, pretrained_model_path, class_num_labels, line_num_labels,
        ...
        # 共享的CodeBERT编码器
        self.codebert = RobertaModel.from_pretrained(pretrained_model_path)
        ...
        self.classification_head = nn.Sequential(
            nn.LayerNorm(expert_dim),
            nn.Linear(expert_dim, class_num_labels)
        )

        self.line_level_head = nn.Sequential(
            nn.LayerNorm(expert_dim),
            nn.Linear(expert_dim, line_num_labels)
        )
    ...
    def get_gate_weights(self, task, **kwargs):
        ...
        if task == "classification":
            ...
            outputs = self.codebert(input_ids=line_ids_flat, attention_mask=attention_mask_flat)
            code_pooled_output = outputs.pooler_output.view(batch_size, seq_len, -1)
    ...
    def forward_line_level(self, line_ids, attention_mask, commit_features=None, expert_features=None):
        """行级任务的前向传播 - 移除提交特征和专家特征的处理"""
        logger = logging.getLogger(__name__)
        
        batch_size, seq_len, token_len = line_ids.shape
        # logger.info(f"输入形状: batch_size={batch_size}, seq_len={seq_len}, token_len={token_len}")

        # 重塑以处理所有代码行
        line_ids_flat = line_ids.view(-1, token_len)
        attention_mask_flat = attention_mask.view(-1, token_len)
        # logger.info(f"扁平化后形状: line_ids_flat={line_ids_flat.shape}")

        # 获取每行的CodeBERT编码
        outputs = self.codebert(input_ids=line_ids_flat, attention_mask=attention_mask_flat)
        code_pooled_output = outputs.pooler_output
        # logger.info(f"CodeBERT输出形状: code_pooled_output={code_pooled_output.shape}")
        
        # 检查形状并确保正确重塑
        expected_flat_size = batch_size * seq_len
        if code_pooled_output.size(0) != expected_flat_size:
            # logger.warning(f"池化输出大小不匹配! 预期: {expected_flat_size}, 实际: {code_pooled_output.size(0)}")
            # 处理不匹配的情况
            actual_flat_size = code_pooled_output.size(0)
            # 如果实际大小小于预期大小，进行填充
            if actual_flat_size < expected_flat_size:
                padding_size = expected_flat_size - actual_flat_size
                hidden_size = code_pooled_output.size(1)
                padding = torch.zeros(padding_size, hidden_size, device=code_pooled_output.device)
                code_pooled_output = torch.cat([code_pooled_output, padding], dim=0)
                # logger.info(f"填充后形状: {code_pooled_output.shape}")
            # 如果实际大小大于预期大小，进行裁剪
            else:
                code_pooled_output = code_pooled_output[:expected_flat_size]
                # logger.info(f"裁剪后形状: {code_pooled_output.shape}")
        
        # 重塑回批次和序列维度
        try:
            code_pooled_output = code_pooled_output.view(batch_size, seq_len, -1)
            # logger.info(f"重塑后形状: {code_pooled_output.shape}")
        except RuntimeError as e:
            # logger.error(f"重塑失败: {e}")
            # 如果重塑失败，记录错误并尝试修复
            hidden_size = code_pooled_output.size(1)
            # 创建一个正确形状的零张量
            correct_shape = torch.zeros(batch_size, seq_len, hidden_size, device=code_pooled_output.device)
            # 尽量填充有效的值
            flat_size = min(code_pooled_output.size(0), batch_size * seq_len)
            for i in range(flat_size):
                b = i // seq_len
                s = i % seq_len
                if b < batch_size and s < seq_len:
                    correct_shape[b, s] = code_pooled_output[i]
            code_pooled_output = correct_shape
            # logger.info(f"手动重构后形状: {code_pooled_output.shape}")

        # 创建行级别的掩码，表示哪些行是有效的（非空）
        line_attention_mask = (attention_mask.sum(dim=2) > 0).float()  # [batch_size, seq_len]
        # logger.info(f"行掩码形状: {line_attention_mask.shape}")

        # 通过行级任务特化层，使用正确维度的掩码
        code_pooled_output = self.line_level_encoder(code_pooled_output, line_attention_mask)
        # logger.info(f"编码后形状: {code_pooled_output.shape}")

        # 强制确保张量为3D
        if len(code_pooled_output.shape) != 3:
            # logger.warning(f"code_pooled_output不是3D! 当前形状: {code_pooled_output.shape}")
            if len(code_pooled_output.shape) == 2:
                # 如果是2D [batch_size*seq_len, hidden_size]，重塑为3D
                try:
                    code_pooled_output = code_pooled_output.view(batch_size, seq_len, -1)
                except RuntimeError:
                    # 如果重塑失败，创建正确形状的零张量并复制数据
                    hidden_size = code_pooled_output.size(1)
                    new_tensor = torch.zeros(batch_size, seq_len, hidden_size, device=code_pooled_output.device)
                    # 只使用第一行的数据复制到所有行
                    for b in range(batch_size):
                        for s in range(seq_len):
                            idx = min(b * seq_len + s, code_pooled_output.size(0) - 1)
                            new_tensor[b, s] = code_pooled_output[idx]
                    code_pooled_output = new_tensor
            # logger.info(f"调整后code_pooled_output形状: {code_pooled_output.shape}")

        # 应用dropout
        fused_features = self.line_level_dropout(code_pooled_output)
        # logger.info(f"处理后特征形状: {fused_features.shape}")

        # 对每行应用MMOE
        line_level_outputs = []
        for i in range(seq_len):
            features = fused_features[:, i, :]
            task_outputs, _ = self.mmoe(features)
            line_level_outputs.append(task_outputs[1])  # 任务1是行级

        # 堆叠所有行的输出
        line_level_output = torch.stack(line_level_outputs, dim=1)
        # logger.info(f"最终行级输出形状: {line_level_output.shape}")

        # 行级预测头
        logits = self.line_level_head(line_level_output)
        # logger.info(f"最终logits形状: {logits.shape}")
        return logits
