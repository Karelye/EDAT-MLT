...
class Config:
    ...
    # 任务损失权重参数 - 控制不同任务损失的比例
    task_loss_weights = {
        "classification": 1.6,  # 分类任务权重
        "line_level": 1.4  # 行级任务权重
    }
    ...
    for batch_idx, batch_data in enumerate(progress_bar):
        ...
        if task == "classification" and classification_batch is not None:
            ...
            if config.use_pgd and pgd is not None:
                ...
                # 应用任务权重
                weighted_loss = loss * config.task_loss_weights["classification"]
                ...
                progress_bar.set_postfix({
                    'loss': f'{loss.item():.4f}',
                    'weight': f'{config.task_loss_weights["classification"]:.2f}',
            ...
            # 从行级批次获取数据 (更新的键名)
            input_ids = line_level_batch['input_ids'].to(config.device)
            attention_mask = line_level_batch['attention_mask'].to(config.device)
            labels = line_level_batch['label'].to(config.device)

            # 正常前向传播 (更新的参数名)
            # 注意: 假设您的模型在处理行级任务时，期望的输入参数是 input_ids, attention_mask
            # 您可能需要根据您的模型定义调整这里的参数传递
            outputs = model(
                task=task,
                input_ids=input_ids,
                attention_mask=attention_mask
                # 如果您的模型行级部分还需要其他特定参数，请在此处添加
            )
            # 使用损失函数计算损失值
            loss = loss_fn(outputs.view(-1, config.line_num_labels), labels.view(-1))

            # 应用任务权重
            weighted_loss = loss * config.task_loss_weights["line_level"]
            ...
            progress_bar.set_postfix({
                'loss': f'{loss.item():.4f}',
                'weight': f'{config.task_loss_weights["line_level"]:.2f}',
                'lr': f'{scheduler.get_last_lr()[0]:.2e}'
            })
...
def train_batch_alternate(model, dataloaders, optimizers, schedulers, loss_fns, config):
    ...
    for _ in range(total_batches * 2):  # 每个原始批次处理两个任务
        if current_task == "classification" and task_batch_count < total_classification_batches:
            ...
            if classification_batch is not None and isinstance(classification_batch, tuple):
                ...
                if batch is not None:
                    ...
                    # 应用任务权重
                    weighted_classification_loss = classification_loss * config.task_loss_weights["classification"]

                    # 反向传播
                    optimizers["classification"].zero_grad()
                    weighted_classification_loss.backward()
                    torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=config.max_grad_norm)
                    optimizers["classification"].step()
                    schedulers["classification"].step()
        ...
        # 修改train_batch_alternate中处理行级任务的部分
            ...
            # 检查批次格式
            if isinstance(line_level_batch, tuple):
                if len(line_level_batch) >= 2:
                    # 假设元组的第二个元素是行级数据
                    line_level_batch = line_level_batch[1]
                    if line_level_batch is None:
                ...
                line_level_batch = line_level_batch
            ...
            attention_mask = line_level_batch['attention_mask'].to(config.device)
            commit_features = line_level_batch.get('commit_features', torch.zeros(1).to(config.device)).to(
                config.device)
            expert_features = line_level_batch['expert_features'].to(config.device)
            ...
            # 使用焦点损失计算损失值
            loss = loss_fns["line_level"](outputs.view(-1, config.line_num_labels), line_labels.view(-1))

            # 应用任务权重
            weighted_loss = loss * config.task_loss_weights["line_level"]

            # 反向传播
            optimizers["line_level"].zero_grad()
            weighted_loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=config.max_grad_norm)
            optimizers["line_level"].step()
            schedulers["line_level"].step()

            # 记录损失
            line_level_losses.append(loss.item())
            ...
            progress_bar.set_postfix({
                ...
                'loss': f'{loss.item():.4f}',
                'weight': f'{config.task_loss_weights["line_level"]:.2f}',
                'lr': f'{schedulers["line_level"].get_last_lr()[0]:.2e}'
            })
    ...
    return {
        ...
        "total_loss": avg_classification_loss * config.task_loss_weights["classification"] +
                      avg_line_level_loss * config.task_loss_weights["line_level"]
    }
...
def train_joint(model, dataloaders, optimizers, schedulers, loss_fns, config, tokenizer=None):
    ...
    for _ in range(total_batches):
            ...
            if batch is not None and 'line_ids' in batch:
                ...
                if config.use_pgd and pgd is not None and tokenizer is not None:
                    ...
                    outputs = model(
                        ...
                        commit_features=batch['commit_features'],
        ...
        if classification_loss != 0 or line_level_loss != 0:
            # 应用任务权重
            weighted_classification_loss = classification_loss * config.task_loss_weights["classification"]
            weighted_line_level_loss = line_level_loss * config.task_loss_weights["line_level"]

            # 计算加权总损失
            total_loss = weighted_classification_loss + weighted_line_level_loss
        ...
        progress_bar.set_postfix({
            ...
            'cls_w': f'{config.task_loss_weights["classification"]:.2f}',
            'line_w': f'{config.task_loss_weights["line_level"]:.2f}',
            'lr': f'{schedulers["classification"].get_last_lr()[0]:.2e}'
        })
        progress_bar.update(1)
...
def main():
    # 添加命令行参数解析
    parser = argparse.ArgumentParser(description='多任务代码漏洞检测训练脚本')
    # 添加任务损失权重参数
    parser.add_argument('--classification_weight', type=float, default=1.0,
                        help='分类任务损失权重，默认为1.0')
    parser.add_argument('--line_level_weight', type=float, default=1.0,
                        help='行级任务损失权重，默认为1.0')
    # 其他可能的参数
    parser.add_argument('--use_alternate_training', action='store_true',
                        help='是否使用交替训练策略')
    parser.add_argument('--alternate_strategy', type=str, choices=['batch', 'epoch', 'progressive'],
                        default='epoch', help='交替训练策略，可选: batch, epoch, progressive')
    parser.add_argument('--device', type=str, default=None,
                        help='使用的设备，例如 "cuda:0" 或 "cpu"')
    parser.add_argument('--output_dir', type=str, default="multi_task_alternate_output",
                        help='输出目录')
    ...
    # 根据命令行参数更新配置
    config.task_loss_weights["classification"] = args.classification_weight
    config.task_loss_weights["line_level"] = args.line_level_weight
    config.use_alternate_training = args.use_alternate_training
    ...
    # 输出训练模式和任务权重信息
    logger.info(f"分类任务损失权重: {config.task_loss_weights['classification']}")
    logger.info(f"行级任务损失权重: {config.task_loss_weights['line_level']}")
