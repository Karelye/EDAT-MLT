...
class MultiTaskVulnerabilityModel(nn.Module):
    ...
    def __init__(self, pretrained_model_path, class_num_labels, line_num_labels,
                 expert_feature_dim=25, expert_num=4, expert_dim=512, max_codeline_length=50):
        ...
        # 专家特征融合层 - 将手工特征与CodeBERT特征融合
        self.expert_feature_dim = expert_feature_dim
        if expert_feature_dim > 0:
            ...
            mmoe_input_dim = hidden_size
        else:
            self.expert_feature_projection = None
    ...
    def forward_line_level(self, line_ids, attention_mask, expert_features=None):
        """行级任务的前向传播"""
        ...
        # 获取每行的CodeBERT编码
        outputs = self.codebert(input_ids=line_ids_flat, attention_mask=attention_mask_flat)
        code_pooled_output = outputs.pooler_output  # [batch_size, hidden_size]
        
        # 应用dropout
        code_encoded = self.line_level_dropout(code_pooled_output)
        
        # 融合专家特征
        if self.expert_feature_dim > 0 and expert_features is not None:
            # 投影专家特征
            expert_projected = self.expert_feature_projection(expert_features)  # [batch_size, hidden_size//2]
            
            # 融合CodeBERT特征和专家特征
            combined_features = torch.cat([code_encoded, expert_projected], dim=1)  # [batch_size, hidden_size + hidden_size//2]
            fused_features = self.feature_fusion(combined_features)  # [batch_size, hidden_size]
        else:
            fused_features = code_encoded

        # 应用MMOE
        task_outputs, _ = self.mmoe(fused_features)
