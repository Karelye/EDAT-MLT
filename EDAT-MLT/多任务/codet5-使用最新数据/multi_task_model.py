...
class MultiTaskVulnerabilityModel(nn.Module):
    ...
    def get_gate_weights(self, task, **kwargs):
        ...
        if task == "classification":
            ...
            # 对CodeT5的输出进行池化操作
            mask_expanded = attention_mask_flat.unsqueeze(-1).expand(last_hidden_state.size()).float()
            sum_embeddings = torch.sum(last_hidden_state * mask_expanded, 1)
            sum_mask = torch.clamp(mask_expanded.sum(1), min=1e-9)
            code_pooled_output = sum_embeddings / sum_mask
    ...
    def forward_classification(self, input_ids, attention_mask):
        ...
        # 对CodeT5的输出进行池化操作（取[CLS]位置或平均池化）
        # 使用attention_mask进行掩码平均池化
        mask_expanded = attention_mask.unsqueeze(-1).expand(last_hidden_state.size()).float()
        sum_embeddings = torch.sum(last_hidden_state * mask_expanded, 1)
        sum_mask = torch.clamp(mask_expanded.sum(1), min=1e-9)
        pooled_output = sum_embeddings / sum_mask
    ...
    def forward_line_level(self, line_ids, attention_mask, expert_features=None):
        ...
        sum_embeddings = torch.sum(last_hidden_state * mask_expanded, 1)
        sum_mask = torch.clamp(mask_expanded.sum(1), min=1e-9)
        code_pooled_output = sum_embeddings / sum_mask  # [batch_size, hidden_size]
