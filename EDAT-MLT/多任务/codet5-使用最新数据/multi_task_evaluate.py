...
def get_line_level_metrics(line_score, label, han_line_score, config=None):
    ...
    logger.info(f"数据统计: 总行数={total_lines}, 漏洞行数={total_vulnerable_lines}, 漏洞行比例={total_vulnerable_lines/total_lines:.4f}")
    ...
    logger.info(f"最终混淆矩阵: TP={best_metrics['tp']}, FP={best_metrics['fp']}, TN={best_metrics['tn']}, FN={best_metrics['fn']}")
...
def get_tokenizer_path(pretrained_path):
    ...
    if os.path.exists(pretrained_path):
        ...
        logger.info("回退到使用HuggingFace Hub模型: Salesforce/codet5-base")
...
def select_features(expert_features, config):
    ...
    if config.feature_selection_strategy == "first":
        ...
        if len(available_important_indices) >= target_dim:
            ...
            # 如果重要特征不够，补充前面的特征
            selected_indices = available_important_indices + [i for i in range(original_dim) if i not in available_important_indices]
...
def apply_ranking_interference(line_df, enable_interference=True, 
    ...
    logger.info(f"干扰参数: shuffle_prob={shuffle_probability}, swap_prob={swap_probability}, group_shuffle_prob={group_shuffle_probability}")
...
def evaluate_line_level(model, dataloader, config):
    ...
    if all_line_scores:
        ...
        # 统计不同分数区间的标签分布
        high_score_indices = [i for i, score in enumerate(all_line_scores) if score > score_stats['mean'] + score_stats['std']]
        low_score_indices = [i for i, score in enumerate(all_line_scores) if score < score_stats['mean'] - score_stats['std']]
...
def main():
    ...
    if config.enable_ranking_interference:
        logger.info(f"启用排序随机干扰，干扰参数: shuffle_prob={config.shuffle_probability}, swap_prob={config.swap_probability}, group_shuffle_prob={config.group_shuffle_probability}")
    else:
