...
def run_ablation_study():
    ...
    experiments = [
        ...
        ("important_15_features", ["--use_limited_features", "--num_features", "15", "--feature_strategy", "important"], f"{results_dir}/important_15_features.txt"),
        ("important_10_features", ["--use_limited_features", "--num_features", "10", "--feature_strategy", "important"], f"{results_dir}/important_10_features.txt"),
        ("important_5_features", ["--use_limited_features", "--num_features", "5", "--feature_strategy", "important"], f"{results_dir}/important_5_features.txt"),
        ("important_3_features", ["--use_limited_features", "--num_features", "3", "--feature_strategy", "important"], f"{results_dir}/important_3_features.txt"),
    ]
...
def run_strategy_comparison():
    ...
    # 不同策略的实验配置（都使用10个特征）
    experiments = [
        ("first_10_features", ["--use_limited_features", "--num_features", "10", "--feature_strategy", "first"], f"{results_dir}/first_10_features.txt"),
        ("random_10_features", ["--use_limited_features", "--num_features", "10", "--feature_strategy", "random", "--feature_seed", "42"], f"{results_dir}/random_10_features.txt"),
        ("important_10_features", ["--use_limited_features", "--num_features", "10", "--feature_strategy", "important"], f"{results_dir}/important_10_features.txt"),
    ]
...
def run_robustness_test():
    ...
    # 模拟不同程度的特征缺失
    experiments = [
        ("severe_loss_1_feature", ["--use_limited_features", "--num_features", "1", "--feature_strategy", "important"], f"{results_dir}/severe_loss_1_feature.txt"),
        ("major_loss_3_features", ["--use_limited_features", "--num_features", "3", "--feature_strategy", "important"], f"{results_dir}/major_loss_3_features.txt"),
        ("moderate_loss_8_features", ["--use_limited_features", "--num_features", "8", "--feature_strategy", "random", "--feature_seed", "123"], f"{results_dir}/moderate_loss_8_features.txt"),
        ("minor_loss_20_features", ["--use_limited_features", "--num_features", "20", "--feature_strategy", "first"], f"{results_dir}/minor_loss_20_features.txt"),
    ]
