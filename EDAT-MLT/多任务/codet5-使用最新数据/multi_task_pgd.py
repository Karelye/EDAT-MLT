...
class PGDAttack:
    ...
    def __init__(self, model, emb_name=None):
        ...
        if not self.emb_name:
            for name, param in self.model.named_parameters():
                # 寻找CodeT5的共享嵌入层或传统的embeddings层
                if ('shared.weight' in name or 'embeddings' in name) and param.requires_grad:
                    self.emb_name = name
                    break
            else:
    ...
    def compute_perturbation(self, epsilon=0.02, alpha=1e-2, n_steps=3):
        """计算对抗扰动，严格遵循原始实现"""
        # 获取嵌入权重
        embeddings = self.model.codet5.shared.weight
    ...
    def apply_perturbation(self, token_ids, perturbation, tokenizer, codes):
        ...
        # 保存原始嵌入
        with torch.no_grad():
            original_embeddings = self.model.codet5.shared.weight[final_target_ids].clone()
            # 应用扰动
            self.model.codet5.shared.weight[final_target_ids] += perturbation[final_target_ids]
    ...
    def restore_embeddings(self, target_ids, original_embeddings):
        ...
        if len(target_ids) > 0:
            with torch.no_grad():
                self.model.codet5.shared.weight[target_ids] = original_embeddings
