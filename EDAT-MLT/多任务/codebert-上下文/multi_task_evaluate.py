...
class EvalConfig:
    ...
    # 行级任务数据路径
    line_level_test_path = r"E:\Python\code_people\最新实验\bigvul\test_line_level_filtered.jsonl"
    ...
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(tqdm(dataloader, desc="评估分类任务")):
            ...
            if return_attention:
                try:
                    # 临时启用输出注意力权重
                    if hasattr(model, 'codebert') and hasattr(model.codebert, 'config'):
                        original_output_attentions = getattr(model.codebert.config, 'output_attentions', False)
                        model.codebert.config.output_attentions = True
                        
                        # 使用模型的codebert获取注意力权重
                        outputs = model.codebert(input_ids=input_ids, attention_mask=attention_mask)
                        
                        if hasattr(outputs, 'attentions') and outputs.attentions and classification_batch.get('original_function'):
                            # 获取注意力权重
                            attention_weights = outputs.attentions  # tuple of [batch_size, num_heads, seq_len, seq_len]
                            attention_weights = torch.stack(attention_weights, dim=1)  # [batch_size, num_layers, num_heads, seq_len, seq_len]
                        ...
                        # 恢复模型配置
                        model.codebert.config.output_attentions = original_output_attentions
                    else:
                        logger.warning(f"模型结构不支持注意力权重提取。模型属性: {[attr for attr in dir(model) if not attr.startswith('_')]}")
...
def build_data_mapping(classification_dataset, line_level_dataset):
    ...
    classification_count = len(classification_dataset.examples) if hasattr(classification_dataset, 'examples') else len(classification_dataset)
    line_level_count = len(line_level_dataset.examples) if hasattr(line_level_dataset, 'examples') else len(line_level_dataset)
                ...
                try:
                    ...
                    if batch_attention_scores:
                        ...
                        if primary_commit_id and primary_commit_id in batch_attention_scores:
                            ...
                            # 融合当前batch的所有行得分
                            for j, (model_score, line_num) in enumerate(zip(line_scores, line_numbers if line_numbers else range(len(line_scores)))):
